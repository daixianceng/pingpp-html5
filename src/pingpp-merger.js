...
PingppSDK.prototype = {
  ...
  createPayment: function(charge_json, callback, signature, debug) {
    ...
    if (channel == channels.upmp_wap) {  // 调起银联支付控件，客户端需要安装银联支付控件才能调起
    ...
    } else if (channel == channels.upacp_pc) {
      ...
      var query = stringify_data(credential, channel, true);
      if (typeof _AP != "undefined") {
        _AP.pay(cfg.ALIPAY_PC_DIRECT_URL + "?" + query);
      } else {
        location.href = cfg.ALIPAY_PC_DIRECT_URL + "?" + query;
      }
    }
  },
...
// aliases
window.PINGPP_PAY_SDK = window.PINGPP_WX_PUB = window.pingppPc = window.pingpp;
})();
